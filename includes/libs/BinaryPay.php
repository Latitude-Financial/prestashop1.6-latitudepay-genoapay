...
abstract class BinaryPay extends Base implements GatewayInterface
{
    ...
    {
        return array(
            'environment', 'ip', 'apiKey', 'password', 'accountId'
        );
    }
    ...
    public function validate($response)
    {
        ...
        foreach ($successStatus as $success) {
            if (empty($success) or strpos($response['body'], $success) !== false) {
                $status += 1;
                break;
            }
        }
    ...
    protected function _runGatewayTest($gateways, $credential)
    {
        if (is_array($gateways)) {
            foreach ($gateways as $gateway) {
                ...
                $paymark = BinaryPay::getGateway($gateway, $credential);
                $paymark->test();
            }
        } else {
            $gateways = ucfirst($gateways);
            echo "\n[-]Starting \033[92m$gateways\033[37m Test:\n";
            $paymark = BinaryPay::getGateway($gateways, $credential);
            $paymark->test();
        }
        return "\nTesing finished.\n";
    }
